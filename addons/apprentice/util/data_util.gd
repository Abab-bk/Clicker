...
##[br][code]data[/code]  返回数据
static func merge(merge_target, data):
	if merge_target is Dictionary:
		merge_target.merge(data)
		return merge_target
	elif merge_target is Array:
		merge_target.append_array(data)
		return merge_target
	elif merge_target is String:
		return merge_target + data

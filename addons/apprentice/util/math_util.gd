...
class N:
	...
	## 获取距离最近的节点
	static func get_closest_node(target: CanvasItem, nodes: Array[CanvasItem]) -> CanvasItem:
		if nodes.is_empty():
			return null
		...
		var node : Node = null
		for child in nodes:
			tmp_dist = target.global_position.distance_squared_to(child.get_global_position())

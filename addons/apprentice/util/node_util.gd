...
## 删除所有子节点
static func queue_free_all_child(parent: Node):
	for child in parent.get_children():
		child.queue_free()
...
class AnimSubControl:
	...
	var _enabled : bool = true:
		set(v):
			if _enabled == false:
				_enabled = v
				_actor.get_button().disabled = true
	
	func _init(actor: Control):
		self._actor = actor
		add_child(actor)
	...
	func execute(state: bool, duration: float = 0.2) -> void:
		if not _enabled:
			return
		...
		if state:
			# 伸出
			self.size.y = 0
			tween.parallel().tween_property(_actor, "position:x", 0, duration)
			tween.parallel().tween_property(self, "custom_minimum_size:y", _actor.size.y, duration)
			
		else:
			# 缩入
			tween.parallel().tween_property(_actor, "position:x", -_actor.size.x, duration)
			tween.parallel().tween_property(self, "custom_minimum_size:y", 0, duration)
		tween.finished.connect(func(): self.finished.emit(state) )


## 创建动画节点
static func create_animation_control(actor: Control):
	return AnimSubControl.new(actor)

...
class Generator:
	...
	static func _rand_list(
		...
		if probability_list.is_empty():
			probability_list.resize(value_list.size())
		
		# 累加概率值，计算概率总和
		# 每次累加存到列表中作为概率区间
		var sum : float = 0.0
		var p_list : Array = []	# 概率列表
...
## 设置生成大小
func set_size(v: Vector2i) -> GenerateRoomUtil:
	size = v
	return self
...
#============================================================
## 生成
func generate(line_count_min := 0, line_count_max := 0) -> Array:
